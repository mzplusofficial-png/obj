...
export const ChallengePresentation: React.FC<ChallengePresentationProps> = ({ isVisible, onAccept, onClose }) => {
  ...
  return (
    <AnimatePresence>
      {isVisible && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none p-4 sm:p-6 overflow-hidden">
          ...
          <motion.div
            ...
            className={`relative pointer-events-auto w-full transition-all duration-500 z-10 ${modalStep === 'timeline' ? 'max-w-2xl' : 'max-w-md'}`}
          >
            <div 
            ...
            >
              ...
              <div className="relative bg-[#080808] rounded-[2.3rem] p-6 md:p-10 flex flex-col items-center text-center overflow-hidden z-10 min-h-[400px] justify-center">
                ...
                <AnimatePresence mode="wait">
                  {modalStep === 'intro' ? (
                    ...
                    <motion.div
                      ...
                      className="w-full flex flex-col items-center"
                    >
                      <h3 className="text-3xl font-black text-white uppercase tracking-tighter mb-4 drop-shadow-lg flex items-center gap-3">
                        <Target className="text-[var(--color-gold-main)]" size={32} />
                        Plan d'Action
                      </h3>
                      <p className="text-neutral-400 text-sm mb-12 uppercase tracking-widest font-bold">Progression du défi 3 Jours</p>

                      {/* Horizontal Timeline */}
                      <div className="w-full relative px-2 sm:px-6 mb-16">
                        {/* Background Tube Base */}
                        <div className="absolute top-[35px] left-8 right-8 h-3 bg-white/5 border border-white/10 rounded-full shadow-inner overflow-hidden">
                          {/* Dark filler */}
                            ...
                            return (
                              <div key={idx} className="flex flex-col items-center flex-1 relative group">
                                ...
                                <motion.div 
                                  ...
                                  className="absolute -top-10 whitespace-nowrap bg-white/5 border border-white/10 px-3 py-1 rounded-full text-[10px] font-mono text-neutral-300 font-bold tracking-widest uppercase shadow-lg group-hover:bg-white/10 transition-colors"
                                >
                                  {dates[idx]}
                                </motion.div>
                                ...
                                {isFirst && (
                                  <motion.div 
                                    ...
                                    className="absolute top-[40px] left-1/2 w-full h-[1px] bg-gradient-to-r from-[var(--color-gold-main)]/50 to-transparent z-0 pointer-events-none"
                                  />
                                )}
